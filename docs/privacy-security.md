...
> Status: **Phase 6.7** — ATTENDANCE HISTORY + CSV EXPORT. PHASE 6.7 adds the Teacher attendance history feature and CSV export. A new server-only read boundary `getAttendanceHistoryForCurrentTeacher(classId)` returns only CLOSED sessions with aggregate counts (presentCount, absentCount, rosterCount) — no rosterSnapshot, no student IDs, no startedByUserId, no teacherUserId are projected. The history list page and detail page (reusing `getAttendanceFinalSummaryForCurrentTeacher`) also project only safe historical identity from the immutable rosterSnapshot. The CSV export route (`GET /api/attendance/export`) exports ONLY: fullName, identificationCode, status, recognizedAt. NEVER exported: studentUserId, AttendanceMark._id, teacherUserId, membershipId, FaceProfile, embeddings, centroids, biometric data. CSV uses UTF-8 BOM for Excel/Vietnamese compatibility. Formula injection is neutralized (cells starting with =, +, -, @ are prefixed with single quote). PHASE 6.7 does NOT modify recognition pipeline (Phase 6.6 semantics sealed), does NOT introduce history mutations, and does NOT install XLSX dependencies.
